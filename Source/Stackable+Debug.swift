...
private extension UIView {
    ...
    func debug_outline(recurse: Bool) {
        ...
        let axIdBlacklist: [String] = [
            UIStackView.tl.axID.space,
            UIStackView.tl.axID.hairline,
            UIStackView.tl.axID.debug.outline,
            UIStackView.tl.axID.debug.margin,
            UIStackView.tl.axID.debug.space,
        ]
    ...
    func debug_showMargins(recurse: Bool) {
        ...
        view.accessibilityIdentifier = UIStackView.tl.axID.debug.margin
        view.translatesAutoresizingMaskIntoConstraints = false
        addSubview(view)
    ...
    func debug_showSpaces(axis: NSLayoutConstraint.Axis) {
        switch self {
        case _ where accessibilityIdentifier == UIStackView.tl.axID.space:
            let axID = UIStackView.tl.axID.debug.space
            guard !subviews.contains(where: { $0.accessibilityIdentifier == axID }) else { return }
    ...
    override func layoutSubviews() {
        ...
        if config.shape.contains(.height) {
            centerXShapeLayer.path = {
                ...
                let start = CGPoint(x: bounds.midX, y: bounds.minY).applying(insetTransform)
                let end = CGPoint(x: bounds.midX, y: bounds.maxY).applying(insetTransform.inverted())
                ...
                path.move(to: start.applying(capTransform))
                path.addLine(to: start.applying(capTransform.inverted()))

                path.move(to: end.applying(capTransform))
                path.addLine(to: end.applying(capTransform.inverted()))
        ...
        if config.shape.contains(.width) {
            centerYShapeLayer.path = {
                ...
                let start = CGPoint(x: bounds.minX, y: bounds.midY).applying(insetTransform)
                let end = CGPoint(x: bounds.maxX, y: bounds.midY).applying(insetTransform.inverted())
